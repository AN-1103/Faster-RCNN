...
# 这里根据原图大小的GTbox和anchor计算出T，其实就是GTbox和anchor之间的横纵坐标偏移和长宽的缩放，然后存储下来。函数最后return 的 np.copy(y_rpn_regr)就是t，而np.copy(y_rpn_cls)就是前景背景的分类记录
...
def calc_rpn(C, img_data, width, height, resized_width, resized_height, img_length_calc_function):
    ...
    # initialise empty output objectives，y_rpn_overlap是记录和GT_BOX重叠高的anchor，y_is_box_valid是记录后面参加不参加分类训练的参数
        ...
        for anchor_ratio_idx in range(n_anchratios):
            ...
            for ix in range(output_width):
                ...
                for jy in range(output_height):
                    ...
                    if bbox_type == 'neg':
                        # 如果是非前景，那么就讲overlap设置为0，但是后面分类的时候需要训练背景，所以就把valid设置为1
                        y_is_box_valid[jy, ix, anchor_ratio_idx + n_anchratios * anchor_size_idx] = 1
                        y_rpn_overlap[jy, ix, anchor_ratio_idx + n_anchratios * anchor_size_idx] = 0
                    elif bbox_type == 'neutral':
                        # 如果iou落在了既不是前景也不是背景的话我们就不要，所以都设置为0
    ...
    # 由于cls是学习前景和背景的，所以这里需要用到y_is_box_valid, y_rpn_overlap
    y_rpn_cls = np.concatenate([y_is_box_valid, y_rpn_overlap], axis=1)
    # 由于regr是回归坐标的，所以这里只需要y_rpn_overlap，即前景的anchor
