...
# 通过rpn的输入是卷积后的feature map 然后经过3*3的卷积后在通过两个卷积把输出分出两个分支，一个就是cls的分支，一个就是reg的分支
def rpn(base_layers, num_anchors):
    #一般是129个前景anchor和128个背景anchor，所以这里可以看到rpn输出是【num_anchors,w,h,tx,ty,tw,th】和【num_anchors,w,h,前景=1，背景=0】
...
def classifier(base_layers, input_rois, num_rois, nb_classes=21, trainable=False):
    ...
    # RoiPoolingConv将不同大小的特征图，压缩成相同大小。比如有两个特征图(16 * 16), 压缩成相同大小(4, 4), 也就是说，对于(16 * 16)的特征图，每(4 * 4)大小的框就做一个maxpooling
